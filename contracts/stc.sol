...
contract SophiaTheCat is ERC721Metadata {
  ...
  constructor(
    string memory baseURI_,
    address admin_
  ) ERC721Metadata("SophiaTheCat", "STC", baseURI_) {
    _admin = admin_;
  }
  ...
  function randomGiveaway(address to, uint256 tokenId) external onlyOwner {
    require(to != address(0), "STC: zero address");    
    _mint(to, tokenId);    
  }
  ...
  function mint(uint256 numberOfNfts) external payable {
    require(!paused, "STC: Minting is currently paused");
    require(pendingCount > 0, "STC: All minted");
    require(numberOfNfts > 0, "STC: numberOfNfts cannot be 0");
    require(
      numberOfNfts <= 20,
      "STC: You may not buy more than 20 NFTs at once"
    );
    require(
      totalSupply().add(numberOfNfts) <= MAX_NFT_SUPPLY,
      "STC: sale already ended"
    );
    require(
  ...
  function _randomMint(address _to) internal returns (uint256) {
    require(totalSupply() < MAX_NFT_SUPPLY, "AvaxFoxes: max supply reached");
